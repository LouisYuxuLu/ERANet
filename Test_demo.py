...
def load_checkpoint(checkpoint_dir,Type,IsGPU):
	...
	if IsGPU == 1:
		...
		model_info = torch.load(checkpoint_dir + Rep +'_checkpoint.pth.tar',map_location=torch.device('cpu'))        
		net = Main()
		device_ids = [0]
...
if __name__ == '__main__': 	
	...
	lr_update_freq = 30
	model,optimizer,cur_epoch = load_checkpoint(checkpoint_dir,Type,IsGPU)
	...
	if IsGPU == 1:
		for f in range(len(testfiles)):
			...
			with torch.no_grad():
				...
				h,w,c = img_c.shape
				img_cc = cv2.resize(img_c,(512,512))
				...
				print('Time:%.4f'%(e-s))    
				E_out = cv2.resize(chw_to_hwc(E_out.squeeze().cpu().detach().numpy()),(w,h))					               
